...
export class TerminalTreeItem extends vscode.TreeItem {
    ...
    constructor(
        ...
        this.tooltip = `${this.label}\nCommande: ${this.terminalCommand}\nDémarrage auto: ${this.autoStart ? 'Oui' : 'Non'}`;
        this.description = this.autoStart ? 'Auto-start' : '';
        
        // Définir une icône différente selon le statut
        this.iconPath = new vscode.ThemeIcon(this.getIcon());
        
        // Ajouter une commande au clic sur le terminal
        this.command = {
            command: 'optidev.executeTerminal',
            title: 'Exécuter le terminal',
            arguments: [this.label, this.terminalCommand]
        } as vscode.Command;

        // Ajouter le contexte pour le menu
        this.contextValue = 'terminal';
    }

    private getIcon(): string {
        const terminal = vscode.window.terminals.find(t => t.name === this.label);
        if (terminal) {
            return 'terminal';
        }
...
export class TerminalTreeProvider implements vscode.TreeDataProvider<TerminalTreeItem> {
    private _onDidChangeTreeData: vscode.EventEmitter<TerminalTreeItem | undefined | null | void> = new vscode.EventEmitter<TerminalTreeItem | undefined | null | void>();
    readonly onDidChangeTreeData: vscode.Event<TerminalTreeItem | undefined | null | void> = this._onDidChangeTreeData.event;
    ...
    async getChildren(element?: TerminalTreeItem): Promise<TerminalTreeItem[]> {
        ...
        const terminalsConfig = vscode.workspace.getConfiguration(this.OPTIDEV).get<{ name: string, command: string, start: boolean }[]>('terminals', []);
        
        return terminalsConfig.map(terminal =>
